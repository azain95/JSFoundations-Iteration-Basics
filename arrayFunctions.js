...
function getOdds(numbers)  {
 let divisionodd = numbers.filter ((numbers)=> numbers % 2 === 1 )
 return divisionodd
}
/**
 * getEvens(numbers):
...
function getEvens(numbers) {
  let divisioneven = numbers.filter ((numbers)=> numbers % 2 === 0 )
  return divisioneven
}
...
function makeThemDoctors(students) {
 let doctors = students => (`Dr. ${students}`)
        return students.map (doctors)
  }
